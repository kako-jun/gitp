...
**実装済み機能**:
- ✅ YAML設定ファイルの読み込み (`gitp/src/setting_util.rs`)
- ✅ `git clone` の実行 (`gitp/src/git_controller.rs:45`)
- ✅ `git pull` の実行 (`gitp/src/git_controller.rs:51`)
- ✅ `git push` の実行 (`gitp/src/git_controller.rs:57`)
- ✅ `git status` の実行 (`gitp/src/git_controller.rs:39`)
...
- ✅ **ratatui フルスクリーンTUI** (`gitp/src/tui.rs`)
...
#### `setting_util.rs`
- `GitpSetting` - 設定データ構造（User, Repos）
- `load()` - gitp_setting.yaml（または.yml）の読み込み（gitp/src/setting_util.rs:42）
