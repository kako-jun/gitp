...
- 複数のリポジトリからの一括clone、一括pull、一括push
- 面倒なコミットコメントの入力を省略

などを実現します

個人で何十個もリポジトリを管理していて、コミットコメントは固定でイイから早く簡単に操作したい、という場合に便利です
...
コミットコメントには、後で他人が振り返れる言葉が求められています

しかし、Gitの使い方は自由です

Privateリポジトリでは、毎回丁寧なコミットコメントを書くのが過剰なケースもあります
...
使わなくなったリポジトリは、`gitp_config.json` から消さなくても `enabled` を `false` にすれば無視されます
...
`gitp` は、この形でリポジトリのディレクトリが並んだ状態を想定しています
...
3. 全てのリポジトリに対して、`git` のコマンドを実行する
4. 1つのリポジトリに対して、`git` のコマンドを実行する
...
`gitp_config.json` で、リモートリポジトリのURLに `ssh`、`https` 両方書いた場合は、`ssh` が優先されます

鍵を登録していない場合は `clone` に失敗します
