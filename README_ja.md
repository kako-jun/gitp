...
個人で多くのリポジトリを管理していて、コミットコメントは固定でイイから早く簡単に操作したい、という場合に便利です
...
`gitp init` で雛形が作られますので、それを書き換えて以下のようにします
...
`gitp_config.json` で `enabled` を `false` にしておいたリポジトリは、スキップされます
